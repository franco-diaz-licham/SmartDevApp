    ...
    <div className="mt-6 grid gap-3">
      ...
      {notes.map((note) => {
        ...
        return (
          <article
            key={note.id}
            className={`rounded-md border p-4 transition hover:border-primary hover:bg-muted/50 ${isSelected ? 'border-primary bg-muted/70' : 'border-border bg-background'}`}
          >
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="flex items-center gap-3 text-sm font-extrabold">
                <Link className="text-primary no-underline hover:underline" to={`/workspace/notes/${encodeURIComponent(note.id)}`} onClick={() => onSelectNote(note.id)}>
                  Open
                </Link>
              </span>
            </div>
