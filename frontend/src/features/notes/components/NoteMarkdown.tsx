import { getUniqueNoteSectionId } from '../utils/noteContent';
...
const renderMarkdownBlock = (block: string, index: number, usedHeadingIds: Map<string, number>) => {
  ...
  if (heading) {
    ...
    return (
      <HeadingTag key={`${title}-${index}`} id={getUniqueNoteSectionId(title, usedHeadingIds)} className="scroll-mt-28 border-t border-border pt-8 text-xl font-extrabold leading-tight first:border-t-0 first:pt-0">
        {title}
      </HeadingTag>
    );
  }
...
export const NoteMarkdown = ({ markdown }: NoteMarkdownProps) => {
